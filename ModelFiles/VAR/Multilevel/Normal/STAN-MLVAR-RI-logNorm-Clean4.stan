...
functions {
  vector lp_reduce( vector beta , vector theta , real[] xr , int[] xi ) {
    int n = 200; // size(xr); // Number of cases per shard
    int K = 2;
    vector[K] Y_std[n];
    vector[K] Xc[n];
    vector[K] Mean[n];
    ...
    matrix[K,K] Omega = to_matrix(theta[((6*n)+1):(((6*n)+1)+3)],K,K); // The correlation matrix for the person in this shard.
    vector[K] tau = theta[((6*n)+5):(((6*n)+5)+1)]; // The sd's for the Y's for the person in this shard.
...
model {
  ...
  vector[1] beta;
  vector[(6*M)+6] theta[n_shards]; // 6M + 8 because two means, two AR-prs, 2 cl-pars, Omega and tau, 6*M if Omega and tau fixed
  ...
  for ( i in 1:n_shards ) {
    ...
    theta[i,((6*M)+1):(((6*M)+1)+3)] = to_vector(Omega[individual[j]]);
    theta[i,((6*M)+5):(((6*M)+5)+1)] = to_vector(tau[individual[j],1:K]);
  }
...
generated quantities {
  ...
  for ( o in 1:I ) {
  betas_raw[o,1,1] = (betas[o,1,1] * sdY[1]/sdX[1]);
  betas_raw[o,1,2] = (betas[o,1,2] * sdY[1]/sdX[2]);
  betas_raw[o,2,1] = (betas[o,2,1] * sdY[2]/sdX[1]);
  betas_raw[o,2,2] = (betas[o,2,2] * sdY[2]/sdX[2]);
  }
